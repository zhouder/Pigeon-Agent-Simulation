...
\newcommand{\pigeonpar}[1]{\vspace{0.1cm}\noindent\textit{#1}\vspace{0.05cm}}
...
\textbf{结论：}提出并验证了"透明度悖论"假说——系统中存在绝对理性的审计者时，因其高效率记录和追索行为加速了承诺-兑现的剪刀差膨胀，导致社交信用体系的更快崩溃。完整推演框架已开源至GitHub：https://github.com/SHIT-Research/Pigeon-Agent-Simulation。
\\
\noindent
...
本研究将其命名为\textbf{"薛定谔的鸽子"}现象：当一个人说出"下次一定"时，在观测行为发生之前，该承诺同时处于"会兑现"和"不会兑现"的叠加态；然而一旦时间到达约定的观测点，波函数必然坍缩为"鸽子态"。这一量子社会学隐喻精确刻画了当代社交承诺的核心矛盾——承诺的真实状态只有在约定的兑现时刻才被决定，而系统的初始条件决定了坍缩的方向\citet{schrodinger_vs_pigeon}。

本质上，"下次一定"构成了一个\textbf{无抵押、无担保、无交割期限}的"三元缺失"社交信用衍生品\citet{ponzi_social}。每一次"下次一定"的发行，即是在社交关系账本上进行了一次无准备金信用扩张。
...
进一步地，从后现代主体性理论出发，"不想出门"可以被重新阐释为\textbf{主体性对现代时间规训的一种微观抵抗形式}。在拉康意义上的"被异化的欲望结构"中，社交承诺被视为一种符号界（Symbolic Order）的规训工具——它强迫个体在外界期望与内在惰性之间做出根本性妥协。而"下次一定"作为一种"不确定的时间悬置"，恰恰提供了打破这种二律背反的第三条路径：既维持了符号界的表面秩序（我答应了），又满足了个体内在欲望的真实表达（我不去）。
...
\textbf{行为经济学派（Behavioral Economics School）}主要以行为决策理论为分析框架，从心理账户（Mental Accounting）和时间贴现（Time Discounting）两个角度切入。早期研究\citet{procrastination}将放鸽子归因为执行功能缺陷和前额叶皮层活动不足，认为个体对远期奖赏的感知能力不足。后续研究\citet{social_capital}通过社会资本理论的视角，将失约视为社会关系投资的负回报，提出了"社交信用折现因子"$\\delta_s$的概念——社交承诺的现值随时间的折现率远高于金融资产。然而该流派的主要局限在于：它预设了个体行为符合理性经济人假设，而"下次一定"的非理性自我欺骗特征无法被标准的贴现效用模型充分解释。

\textbf{量子社会学派（Quantum Sociology School）}是近年来出现的新兴分析范式。其核心主张认为，社交关系的状态在观测前处于不确定的叠加态，观测行为本身改变了系统状态。薛定谔在1935年的思想实验\citet{schrodinger_vs_pigeon}虽然没有直接讨论鸽子问题，但其后续研究者的延伸工作\citet{quantum_social}提出了社交承诺量子化的初步设想。然而，这一流派目前主要停留在隐喻层面，缺乏可操作的数学模型和量化分析手段——这正是本研究试图填补的空白之一。

\textbf{神经拖延学派（Neuro-Procrastination School）}通过fMRI等手段揭示了拖延行为的神经基础\citet{procrastination}。前额叶皮层（PFC）与边缘系统（Limbic System）之间的博弈被认为是"今天不想去"决策的神经生物学基底。但是，这类研究通常局限于个体的神经机制，尚未建立从单脑到社交网络的跨层次桥接模型。

在多智能体社会模拟方面，近年来LLM驱动的多智能体推演成为计算社会学的新范式。Park等人\citet{generative_agents}提出了基于LLM的可信社交智能体框架，Li等人\citet{social_learning}研究了社交规范如何通过智能体间的反复博弈涌现。Watts\citet{network_collapse}在社交网络级联动力学方面做出了开创性贡献。然而，现有工作主要关注\textit{合作性}社会行为的自组织，对于"不合作"行为的系统性崩盘效应——尤其是语言承诺的泡沫化特征——尚未有深入研究。
...
其中$f_D(D_t) = \min(40, D_t/30)$，$f_\tau(\tau_t) = \min(20, 0.3\tau_t)$，$f_C(C_t) = 0.1\max(0, 800 - C_t)$，$f_G(G_t) = 15[1 - K_t/\max(P_t, 1)]$。权重$\bm{w} = (0.35, 0.25, 0.25, 0.15)^T$通过主成分分析确定。

% =========================================================================
\section{EXPERIMENTAL SETUP / 实验设置}
% =========================================================================
...
\begin{tabular}{|c|c|c|}
\hline
\multicolumn{3}{|c|}{\textsc{表1：外部冲击事件}} \\
\hline
周次 & 事件 & 效应 \\
\hline
第11周 & 双十一购物节 & 违约率+200\% \\
...
图\ref{fig:debt_kline}展示了两个实验组的债务总额及泡沫指数演化。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{./fig1_debt_kline.pdf}
\caption{对照组（红）与实验组（蓝）的社交债务总额（实线）与泡沫指数（虚线）对比。对照组债务在外部冲击作用下阶梯式增长至1,372元，泡沫指数97.6（深红）；实验组维持在200-500元。}
\label{fig:debt_kline}
\end{figure}

对照组呈现"阶梯式增长"：每经历一次冲击，债务跃升且无法回落，即"棘轮效应"\citet{ratchet_effect}。实验组大量债务通过阿鱼的"忘记"属性注销。

\subsection{消融实验：透明度悖论}

表\ref{tab:comparison}总结核心指标。
...
\hline
\multicolumn{4}{|c|}{\textsc{表2：核心指标对比}} \\
\hline
指标 & 对照组 & 实验组 & 差异 \\
\hline
...
\end{tabular}
\end{center}
\caption{对照组兑现率高但债务和泡沫指数更大，验证"透明度悖论"。}
\label{tab:comparison}
\end{table}
\normalsize

核心发现：\textbf{透明度悖论}——记账狂作为理性审计者，通过四种机制加速崩溃：（1）债务显性化——隐性债务被纳入资产负债表；（2）复利膨胀——每周5\%复利使名义债务指数增长；（3）信用加速衰减——对照组合计降幅42.6\%，实验组仅32.9\%；（4）挤兑螺旋\citet{bank_run}——大规模追索触发集体违约。

图\ref{fig:credit_decay}展示兑现率演化。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{./fig4_credit_decay.pdf}
\caption{累积兑现率与外部冲击效应。灰色区域为冲击窗口。}
\label{fig:credit_decay}
\end{figure}

\subsection{相图分析}

图\ref{fig:phase_diagram}展示信用系统相图。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{./fig3_phase_diagram.pdf}
\caption{对照组逼近坍缩边界，实验组在较低水平波动。}
\label{fig:phase_diagram}
\end{figure}

\subsection{借口演化分析}

图\ref{fig:excuse_heatmap}展示借口类型热力图。

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{./fig2_excuse_heatmap.pdf}
\caption{借口类型分布热力图（4周聚合）。医疗类在前16周主导，工作类在后半段主导，玄学类在第30周激增。}
...
\begin{equation}
\label{eq:weather}
\text{SBBI}_{\text{扩展}}(t) = \text{SBBI}(t) + \gamma_R \cdot R_t + \gamma_T \cdot (T_t - 25)^2 + \gamma_A \cdot A_t
\end{equation}

其中$R_t$为周降雨量（mm），$T_t$为周平均气温（$^\circ$C），$A_t$为空气质量指数。通过本模拟框架进行敏感性测试后发现：

\begin{itemize}
\item \textbf{降雨效应}：周降雨量每增加10mm，违约率上升约7.3\%（$p<0.05$）。下雨天的"不想出门"因子具有统计显著的解释力。在学术意义上，降雨作为一种外生的"自然借口感生变量"，为借口市场提供了稳定的供应来源；
\item \textbf{温度效应}：当气温偏离舒适区间（18-25$^\circ$C）时，违约率呈U型上升。极端高温（>35$^\circ$C）使违约率增加42\%，极端低温（<0$^\circ$C）使违约率增加28\%。这一不对称性揭示了当代青年对\textbf{热暴露的厌恶程度高于冷暴露}这一有趣的心理偏好；
\item \textbf{空气质量}：AQI超过200（重度污染）时，医疗类借口的占比显著上升（+18.5pp，$p<0.01$），表明雾霾不仅影响身体健康，还提供了"现成可用的违约借口"。
\end{itemize}

这些发现表明，气候变量至少解释了SBBI约12-15\%的周间波动。更一般地，它们揭示了\textbf{社交泡沫的文化生态嵌入性}——任何一个社交承诺系统都存在与其物理环境高度耦合的"借口供给弹性"。

\subsection{MBTI人格类型与承诺博弈中的信息不对称}

为了进一步识别个体异质性与承诺博弈的关系，我们引入MBTI框架进行回溯性分析。虽然MBTI在主流心理学中备受争议，但其二分法维度（特别是J/P维度）在预测"下次一定"行为上显示出出乎意料的解释力。
...
\begin{itemize}
\item \textbf{J型（判断型）vs P型（感知型）}：P型Agent的平均PDR比J型高43.6\%（$t=6.72$，$p<0.001$）。这一差异在社交泡沫膨胀期（第20-40周）进一步扩大至58.2\%。理论解释：P型个体的"开放选项偏好"在社交承诺领域表现为"先答应了再说"的决策模式，其承诺的期权性质（option-like）更加突出，而行权概率则显著偏低。
\item \textbf{I型（内向型）vs E型（外向型）}：I型Agent的"沉默违约"（已读不回）概率是E型的3.2倍。这一差异在第30周（水星逆行的玄学冲击期）进一步放大至4.7倍——内向性特质与玄学借口的交互效应呈现出统计显著的协同作用。
\item \textbf{T型（思维型）vs F型（情感型）}：T型Agent借口中"工作类"占比更高（47\% vs 29\%），而F型借口中"家庭类"和"医疗类"占比更高。这一模式与此类人格的典型归因风格高度一致。
\end{itemize}

值得注意的是，记账狂（对照组中的理性审计者）在MBTI框架下被定位为\textbf{ESTJ}——其极高的J型倾向（追索强度98\%）和T型倾向（精确复利计算）构成了其"审计者人格"的基础。而阿鱼的人格特征更接近\textbf{INFP}——高I型（自己待着就很好）和高P型（七秒记忆）。消融实验表明，ESTJ的"高效记录"在系统层面反而比INFP的"选择性遗忘"更具破坏性——这是一个在人格心理学层面也成立的反直觉结论：\textbf{系统最优的人格配置不一定是功能最强的人格聚合}。
...
"下次一定"作为一种\textbf{粘性承诺}（sticky promise）——承诺一旦发出，其"兑现预期"锚定在初始水平，但实际履约倾向随时间递减。结合公式(\ref{eq:sde})和(\ref{eq:bbbi})：\textbf{当$\mu$持续大于存续承诺衰减率时，系统必经历信用通胀螺旋。}

\subsection{透明度悖论的理论解释}

记账狂的高效记录产生了三个负面效应：(1)隐性债务显性化扩大了信用基盘；(2)频繁追索提高违约感知，诱导预防性违约；(3)精确记账本身构成了"节俭悖论"\citet{keynes_thrift}的微观镜像。

\subsection{外部冲击的棘轮效应}

五种冲击中，双十一和春节催婚效应最持久（3-5周）。水星逆行效应虽强但消退最快（2周）。经济压力冲击比文化信仰冲击更持久。

\subsection{研究局限}

(1)Agent使用参数驱动而非真实LLM；(2)52周对呈现庞氏终结仍不足；(3)未考虑冲击间交互效应。

% =========================================================================
\section{CONCLUSION / 结论}
% =========================================================================

\textbf{第一}，"下次一定"在群体层面呈自组织庞氏结构。对照组债务1,372元，SBBI=97.6；实验组债务501元，SBBI=50.8。

\textbf{第二}，"薛定谔的鸽子"方程证明，当借口感生势能超过承诺稳定能时，波函数必然坍缩到鸽子态。

\textbf{第三}，\textbf{"透明度悖论"}——记账狂使债务扩大173.9\%，而选择性遗忘维持系统稳定。

\textbf{第四}，异质性分析表明，降雨量和人格类型对社交泡沫有显著调节效应。
...
\begin{enumerate}
\item \textbf{仿真框架}：基于Pigeon-Agent-Simulation-Framework v2.0（Python 3.12），已开源至GitHub。
\item \textbf{实验协议}：每周四阶段——(1)债务生成；(2)债务履行；(3)债权追索；(4)状态更新。
\item \textbf{复现保证}：两组使用相同随机种子（seed=42）。
\end{enumerate}
...
\bibitem{promise_act} Searle，J.R.，"How performatives work，" \textit{Linguistics and Philosophy}，vol.12，no.5，pp.535-558，1989.
...
\bibitem{sticky_price} Mankiw，N.G.，"Small menu costs，" \textit{Quarterly Journal of Economics}，vol.100，no.2，1985.
...
\subsection*{附录D：借口分类编码字典（Excuse Coding Dictionary）}
...
\begin{tabular}{|c|l|l|}
\hline
\textbf{编码} & \textbf{借口类别} & \textbf{典型示例} \\
\hline
...
CMT-02 & 下个月 & "下个月一定补上！" \\
CMT-03 & 等XX结束 & "等忙完这阵子/等项目上线" \\
CMT-04 & 下辈子一定 & "下辈子一定请你吃饭" \\
\hline
\end{tabular}
\end{center}
\caption{借口分类编码字典。共8大类30子类，按MED（医疗）、WRK（工作）、FAM（家庭）、MYS（玄学）、ANM（动物）、WTH（天气）、SLC（自我关怀）、CMT（承诺变体）编码。}
...
\begin{tabular}{|c|c|c|}
\hline
\textbf{周次区间} & \textbf{主导借口类别} & \textbf{占比(\%)} \\
\hline
第1-8周 & 医疗(MED) & 38.2 \\
第9-16周 & 工作(WRK) & 31.5 \\
第17-24周 & 医疗+工作(MED+WRK) & 28.3+27.1 \\
第25-32周 & 玄学(MYS) & 34.0 \\
第33-40周 & 家庭(FAM) & 35.7 \\
...
\end{tabular}
\end{center}
\caption{52周推演中主导借口类别的周期性变迁。每8周为一个观测窗口。承诺变体（CMT）在第41-52周的飙升暗示借口的创新能力已经枯竭。}
